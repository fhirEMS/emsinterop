...
**Division of labor:** a thin **emsinterop mapper** (new code we build) owns everything NEMSIS-specific up to a validated FHIR transaction Bundle; **fhirEngine** (exists) owns FHIR validation, persistence, indexing, terminology, MPI, security, and serving. The seam between them is the FHIR REST API — nothing more.

```
  ┌──────────────── emsinterop mapper (Python — new) ─────────────────┐      ┌──────────── fhirEngine (exists) ────────────┐
